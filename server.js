...
if (p.hp <= 0) {
  ...
  p.score++;
    ...
    const alivePlayers = Object.values(room.players).filter(pp => pp.alive);
    if (alivePlayers.length === 1) {
      startNextRound(room, alivePlayers[0].id);
    }
  }
}
          hit = true;
...
const timeLeft = Math.max(0, Math.floor((room.roundEndTime - Date.now()) / 1000));
